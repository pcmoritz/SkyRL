...
@dataclass
class KVCache:
    """Key-value cache for all layers in list format.

    Attributes:
        keys: List of key caches, one per layer. Each has shape (batch, seq, num_kv_heads, head_dim).
        values: List of value caches, one per layer. Each has shape (batch, seq, num_kv_heads, head_dim).
    ...
    def from_layer_outputs(
        ...
        Args:
            keys: List of keys, one per layer. Each has shape (batch, seq, num_kv_heads, head_dim).
            values: List of values, one per layer. Each has shape (batch, seq, num_kv_heads, head_dim).
    ...
    def update_layer(
        ...
        This is called from within the layer loop to update a single layer's cache.

        Args:
            kv_cache: Tuple of (k_cache, v_cache) for this layer.
                      Each has shape (batch, seq, num_kv_heads, head_dim).
            k: New key values of shape (batch, seq_len, num_kv_heads, head_dim).
            v: New value values of shape (batch, seq_len, num_kv_heads, head_dim).
            positions: Position indices of shape (batch, seq_len).

        Returns:
            Updated (k_cache, v_cache) tuple with new values at positions.
        """
    ...
    def pad_to_length(self, max_length: int) -> KVCache:
        """Pad KV cache to a specified maximum sequence length.
        ...
        # Pad only the sequence dimension (axis 1)
